...
export default function TestFieldManager() {
  ...
  const testUser = useQuery(api.users.getTestUser);
  const fields = useQuery(api.fields.getUserFields, 
    testUser ? { userId: testUser._id } : "skip"
  ) || [];
  ...
  const handleCreateField = async () => {
    if (!newFieldData.name || !testUser) return;
  ...
  const handleUpdateFieldCropAreas = async (cropAreas: CropArea[]) => {
    if (!selectedField || !testUser) return;
    ...
    try {
      ...
      console.log('Successfully updated field with crop areas:', cropAreas);
    } catch (error) {
      console.error('Error updating field:', error);
      alert('Failed to update field. Error: ' + error);
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded mb-4">
        <strong>Test Mode:</strong> Authentication bypassed for testing field management functionality.
      </div>
      
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold">Field Management (Test Mode)</h1>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  disabled={!newFieldData.name || isCreating}
                  className="flex-1"
                >
                  {isCreating ? (
                    <>
